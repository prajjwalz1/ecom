...
class ShippingDetails(CustomizedModel):
    order = models.OneToOneField(
        "Order", on_delete=models.DO_NOTHING, related_name="shippingdetails"
    )
...
class PromoCode(CustomizedModel):
    ...
    code = models.CharField(
        max_length=20, unique=True, help_text="Unique code for the promo."
    )
    ...
    discount_percentage = models.FloatField(
        help_text="Discount in percentage only applicable when using Discount Type 'PERCENTAGE' else make it zero",
        default=0,
    )
    ...
    start_date = models.DateTimeField(
        default=timezone.now, help_text="Start date for the promo code."
    )
    end_date = models.DateTimeField(
        default=timezone.now, help_text="End date for the promo code."
    )
    is_active = models.BooleanField(
        default=True, help_text="Is the promo code currently active?"
    )
    count = models.IntegerField(default=0, editable=False)
    limit_users = models.BooleanField(
        default=False, help_text="Is the promo code limited in number?"
    )
...
class Order(CustomizedModel):
    qr_payment_slip = models.ForeignKey(
        "PaymentProof", on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    order_status = models.CharField(
        choices=(
            ...
            ("next_delivery", "Ready to deliver"),
            ("shipped", "shipped"),
    ...
    promocode_used = models.ForeignKey(
        PromoCode, null=True, blank=True, on_delete=models.DO_NOTHING
    )
    ...
    payment_mode = models.CharField(
        choices=(("COD", "COD"), ("mobile_banking", "mobile banking")),
...
class PaymentProof(CustomizedModel):
    ...
    def __str__(self):
        try:
            order = (
                Order.objects.filter(qr_payment_slip__id=self.id)
                .order_by("-created_at")
                .first()
            )
            return (
                order.shippingdetails.phonenumber if order.shippingdetails else "None"
            )
