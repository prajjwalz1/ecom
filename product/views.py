...
from .mixins import *
from .models import Brand, CarouselImage, Category, Navbar, Product, Specification, Tag
...
class HomeView(ResponseMixin, APIView):
    def get(self, request):
        ...
        if request_type == "all_tagged_product":
            ...
            return ResponseMixin.handle_error_response(
                error_message="bad request", status_code=400
            )

    def getnavbar(self, request):
        obj = Navbar.objects.prefetch_related("category").order_by("priority")
    ...
    def AllTaggedProduct(self, request):
        ...
        tags = (
            ...
            .all()
            .order_by("priority", "-last_modified")
        )
        ...
        for tag in serialized_data:
            ...
            section_wise_data[section_name].append(
                {"tag_name": tag_name, "products": products}
            )
        brands = Brand.objects.all()
        brands_serializer = BrandSerializer(
            brands, context={"request": request}, many=True
        )
        ...
        return ResponseMixin.handle_success_response(
            serialized_data=response_data, status_code=200
        )
...
class ProductDetailView(ResponseMixin, APIView, GetSingleObjectMixin):
    ...
    def get(self, request):
        ...
        if request_type == "get_product_detail":
            ...
            return ResponseMixin.handle_error_response(
                error_message="request is not valid", status_code=400
            )
    ...
    def get_product_detail(self, request, id):
        ...
        return ResponseMixin.handle_success_response(
            serialized_data=serializer.data, status_code=200
        )
        breakpoint()
...
class CategoryWiseProduct(APIView, ResponseMixin):
    ...
    def CategoryWiseProduct(self, request):
        ...
        try:
            ...
            child_category_ids = Category.objects.filter(
                parent_category=main_category
            ).values_list("id", flat=True)
            category_ids = list(child_category_ids) + [
                main_category.id
            ...
            products = (
                ...
                .prefetch_related("variants", "variants__productvariantsimages")
            )
            ...
            return ResponseMixin.handle_success_response(
                serialized_data=[], status_code=404, message="Category not found"
            )
...
class ProductReviewView(APIView, ResponseMixin):
    ...
    def post_review(self, request):
        ...
        if review_serializer.is_valid():
            ...
            return ResponseMixin.handle_success_response(
                message="product review added successfully", status_code=200
            )
        else:
            return ResponseMixin.handle_serializererror_response(
                serializer_errors=review_serializer.errors, status_code=400
            )
    ...
    def get_product_review(self, request):
        ...
        return ResponseMixin.handle_success_response(
            ...
            status_code=200,
            serialized_data=review_serializer.data,
        )
    ...
    def BrandwiseProduct(self, request):
        ...
        if not brand_id:
            return self.handle_error_response(
                error_message="brand id not sent in query", status_code=400
            )
...
class ProductDependencies(APIView, ResponseMixin):
    ...
    def get(self, request):
        ...
        if request_type == "get-brands-cat-tags":
            ...
            return self.handle_error_response(
                error_message="bad request type", status_code=400
            )
    ...
    def ProductCrudDependencies(self, request):
        ...
        return self.handle_success_response(
            serialized_data=response_data, status_code=200, message="success"
        )
...
class CategorySpecificationsView(APIView, ResponseMixin):
    ...
    def get(self, request):
        ...
        return self.handle_success_response(
            serialized_data=serializer.data, status_code=200, message="success"
        )
...
class ProductRetrieveUpdateDestroyView(
    ResponseMixin, generics.RetrieveUpdateDestroyAPIView
):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]
    queryset = Product.objects.all()
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.handle_success_response(
            status_code=status.HTTP_200_OK, serialized_data=serialized_data
        )
...
class ProductImageWithoutVariantUpdateDestroyView(
